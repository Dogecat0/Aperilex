...
export function FinancialMetricsGrid({
  ...
  const determineMetricType = (name: string, value: string | number | null | undefined): MetricType => {
    const nameLower = name.toLowerCase()
    
    // Check ratio first to avoid currency detection conflict with "debt"
    if (nameLower.includes('ratio') || nameLower.includes('times') || nameLower.includes('multiple')) {
      return 'ratio'
    }
    
    if (nameLower.includes('margin') || nameLower.includes('percentage') || nameLower.includes('rate') ||
        nameLower.includes('yield') || nameLower.includes('%') || 
        (typeof value === 'string' && value.includes('%'))) {
      return 'percentage'
    }
    
    if (nameLower.includes('revenue') || nameLower.includes('income') || nameLower.includes('cash') || 
        nameLower.includes('assets') || nameLower.includes('debt') || nameLower.includes('$')) {
      return 'currency'
    }
  ...
  const calculateChange = (
    ...
    if (!change && !changeText && current && previous) {
      const currentNum = typeof current === 'number' ? current : parseFloat(String(current).replace(/[^\d.-]/g, ''))
      const previousNum = typeof previous === 'number' ? previous : parseFloat(String(previous).replace(/[^\d.-]/g, ''))
  ...
  const displayMetrics = maxDisplayCount && !showAllMetrics 
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {displayMetrics.map((metric) => {
          ...
          const hasDetailedInfo = metric.explanation || metric.interpretation || metric.industryBenchmark
      ...
      {process.env.NODE_ENV === 'development' && (
        <details className="mt-4">
          <summary className="text-xs text-gray-500 cursor-pointer">Debug: Processed Metrics</summary>
