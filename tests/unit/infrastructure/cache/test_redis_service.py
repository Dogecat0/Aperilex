...
class TestRedisServiceConnection:
    ...
    async def test_connect_failure(self, mock_logger, mock_redis):
        ...
        assert service._connected is False
        mock_logger.error.assert_called_once_with("Failed to connect to Redis: Connection failed")
...
class TestRedisServiceSetOperation:
    ...
    @patch('src.infrastructure.cache.redis_service.logger')
    async def test_set_success_without_expiration(self, mock_logger, mock_ensure_connected):
        ...
        service._redis.set.assert_called_once_with("test_key", json.dumps(test_data, default=str))
        mock_logger.debug.assert_called_once_with("Cached value for key: test_key")

    @patch.object(RedisService, '_ensure_connected')
    @patch('src.infrastructure.cache.redis_service.logger')
    async def test_set_success_with_expiration(self, mock_logger, mock_ensure_connected):
    ...
    async def test_set_failure(self, mock_logger, mock_ensure_connected):
        ...
        mock_logger.error.assert_called_once_with("Failed to set cache key test_key: Redis error")
...
class TestRedisServiceGetOperation:
    ...
    async def test_get_failure(self, mock_logger, mock_ensure_connected):
        ...
        assert result is None
        mock_logger.error.assert_called_once_with("Failed to get cache key test_key: Redis error")
...
class TestRedisServiceDeleteOperation:
    ...
    async def test_delete_failure(self, mock_logger, mock_ensure_connected):
        ...
        assert result is False
        mock_logger.error.assert_called_once_with("Failed to delete cache key test_key: Redis error")
...
class TestRedisServiceExistsOperation:
    ...
    async def test_exists_failure(self, mock_logger, mock_ensure_connected):
        ...
        assert result is False
        mock_logger.error.assert_called_once_with("Failed to check cache key test_key: Redis error")
...
class TestRedisServiceClearPatternOperation:
    ...
    async def test_clear_pattern_success(self, mock_logger, mock_ensure_connected):
        ...
        mock_logger.info.assert_called_once_with("Deleted 3 keys matching pattern: test:*")
    ...
    async def test_clear_pattern_failure(self, mock_logger, mock_ensure_connected):
        ...
        assert result == 0
        mock_logger.error.assert_called_once_with("Failed to clear pattern test:*: Redis error")
...
class TestRedisServiceIncrementOperation:
    ...
    async def test_increment_failure(self, mock_logger, mock_ensure_connected):
        ...
        assert result is None
        mock_logger.error.assert_called_once_with("Failed to increment cache key counter_key: Redis error")
...
class TestRedisServiceHashOperations:
    ...
    async def test_set_hash_failure(self, mock_logger, mock_ensure_connected):
        ...
        assert result is False
        mock_logger.error.assert_called_once_with("Failed to set hash hash_key: Redis error")
    ...
    async def test_get_hash_failure(self, mock_logger, mock_ensure_connected):
        ...
        assert result is None
        mock_logger.error.assert_called_once_with("Failed to get hash hash_key: Redis error")
...
class TestRedisServiceHealthCheck:
    ...
    async def test_health_check_failure(self, mock_logger, mock_ensure_connected):
        ...
        assert result is False
        mock_logger.error.assert_called_once_with("Redis health check failed: Connection lost")

    @patch.object(RedisService, '_ensure_connected')
    @patch('src.infrastructure.cache.redis_service.logger')
    async def test_health_check_connection_error(self, mock_logger, mock_ensure_connected):
        ...
        assert result is False
        mock_logger.error.assert_called_once_with("Redis health check failed: Cannot connect")
...
class TestRedisServiceErrorScenarios:
    ...
    async def test_operations_with_connection_issues(self):
        ...
        service._ensure_connected = AsyncMock(side_effect=Exception("Connection failed"))
