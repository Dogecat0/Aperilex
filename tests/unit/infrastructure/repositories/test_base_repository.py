...
class TestBaseRepositoryIntegration:
    ...
    async def test_error_recovery_with_rollback(self):
        ...
        session = Mock(spec=AsyncSession)
        session.add = Mock()
        session.flush = AsyncMock(side_effect=Exception("Database constraint violation"))
        session.rollback = AsyncMock()
        model_class = MockModel
...
class TestBaseRepositoryEdgeCases:
    ...
    async def test_operations_with_none_values(self):
        ...
        assert entity_with_none.name is None, f"Expected None, got {entity_with_none.name}"
