"""Unit tests for Google Provider with comprehensive API mocking."""
...
from src.infrastructure.llm.google_provider import GoogleProvider, sentiment_to_score
...
class TestGoogleProvider:
    """Test cases for GoogleProvider with mocked API calls."""

    @pytest.fixture
    def mock_settings(self):
        """Mock settings for Google configuration."""
        with patch("src.infrastructure.llm.google_provider.settings") as mock_settings:
    ...
    def provider(self, mock_settings):
        """Create Google provider with mocked dependencies."""
        with patch(
    ...
    def test_init_with_explicit_credentials(self):
        """Test Google provider initialization with explicit credentials."""
        with patch(
            "src.infrastructure.llm.google_provider.genai.Client"
        ) as mock_genai_client:
            provider = GoogleProvider(
                api_key="explicit-google-key",
                model="dummy",
            )

            mock_genai_client.assert_called_once_with(api_key="explicit-google-key")
            assert provider.model == "dummy"

    def test_init_with_settings_credentials(self, mock_settings):
        """Test Google provider initialization using settings."""
        with patch(
            "src.infrastructure.llm.google_provider.genai.Client"
        ) as mock_genai_client:
            _ = GoogleProvider()

            mock_genai_client.assert_called_once_with(api_key="test-google-api-key")
            # Note: The provider initialized before mock, so it uses actual settings
            # This test verifies the provider uses settings correctly for API key
    ...
    async def test_extract_subsection_text_success(
        ...
        mock_response = Mock()
        mock_response.text = "Extracted relevant text for operational overview."
        ...
        assert "Extracted relevant text for operational overview." in result
    ...
    @pytest.mark.asyncio
    async def test_analyze_subsection_success(self, provider: GoogleProvider) -> None:
        ...
        ):  # 100ms difference with extra call
    ...
    @pytest.mark.asyncio
    async def test_analyze_subsection_api_error(self, provider: GoogleProvider) -> None:
    ...
    @pytest.mark.asyncio
    async def test_analyze_section_success(self, provider: GoogleProvider) -> None:
    ...
    @pytest.mark.asyncio
    async def test_analyze_filing_comprehensive(self, provider: GoogleProvider) -> None:
        ...
        mock_overall_response.text = json.dumps(
            {
        ...
        mock_overall_response.usage_metadata = Mock()
        mock_overall_response.usage_metadata.prompt_token_count = 100
        mock_overall_response.usage_metadata.candidates_token_count = 50
    ...
    def test_model_configuration(self, provider: GoogleProvider) -> None:
        ...
        with patch("src.infrastructure.llm.google_provider.genai.Client"):
            ...
            assert custom_provider.model == "dummy"

    @pytest.mark.asyncio
    async def test_concurrent_section_analysis(self, provider: GoogleProvider) -> None:
