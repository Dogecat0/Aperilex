...
class TestRateLimiter:
    ...
    @pytest.mark.asyncio
    async def test_check_rate_limit_within_limits(self, rate_limiter: RateLimiter) -> None:
    ...
    @pytest.mark.asyncio
    async def test_check_rate_limit_per_minute_exceeded(self, rate_limiter: RateLimiter) -> None:
    ...
    @pytest.mark.asyncio
    async def test_check_rate_limit_per_hour_exceeded(self, rate_limiter: RateLimiter) -> None:
    ...
    def test_cleanup_old_calls(self, rate_limiter: RateLimiter) -> None:
        ...
        old_minute_call = now - timedelta(minutes=2)  # Outside minute window, inside hour window
...
class TestExternalServiceCoordinator:
    ...
    async def test_call_edgar_service_success(
        ...
        result = await coordinator.call_edgar_service(mock_service_method, "arg1", kwarg1="value1")
    ...
    async def test_call_llm_service_success(
        ...
        result = await coordinator.call_llm_service(mock_service_method, "arg1", kwarg1="value1")
    ...
    async def test_call_external_service_max_retries_exceeded(
        ...
        # Mock asyncio.sleep to avoid actual delays in tests
        with patch("asyncio.sleep"):
            with pytest.raises(ExternalServiceError, match="edgar service failed after 4 attempts"):
    ...
    async def test_call_external_service_rate_limit_max_retries(
        ...
        with patch.object(
            ...
            with patch("asyncio.sleep"):
                with pytest.raises(ExternalServiceError, match="edgar rate limit exceeded"):
                    await coordinator.call_edgar_service(mock_method)

        # Service method should not have been called due to persistent rate limiting
        mock_method.assert_not_called()

    def test_update_service_status_success(self, coordinator: ExternalServiceCoordinator) -> None:
    ...
    def test_update_service_status_error(self, coordinator: ExternalServiceCoordinator) -> None:
        """Test updating service status on error."""
        coordinator._update_service_status("test_service", "error", "Test error message")
    ...
    def test_update_service_status_consecutive_errors(self, coordinator: ExternalServiceCoordinator) -> None:
    ...
    def test_get_service_health_healthy_service(self, coordinator: ExternalServiceCoordinator) -> None:
    ...
    def test_get_service_health_degraded_service(self, coordinator: ExternalServiceCoordinator) -> None:
    ...
    def test_get_service_health_unhealthy_service(self, coordinator: ExternalServiceCoordinator) -> None:
    ...
    def test_get_service_health_error_rate_calculation(self, coordinator: ExternalServiceCoordinator) -> None:
