...
class TestBackgroundTaskCoordinator:
    ...
    def mock_analysis(self) -> MagicMock:
        ...
        analysis.get_section_analyses.return_value = [
            "section1",
            "section2",
            "section3",
        ]
    ...
    async def test_queue_filing_analysis_success(
        ...
        mock_analysis_orchestrator.orchestrate_filing_analysis.return_value = (
            mock_analysis
        )
    ...
    async def test_queue_filing_analysis_with_progress_tracking(
        ...
        for i, (expected_progress, expected_message) in enumerate(
            expected_progress_calls
        ):
    ...
    async def test_queue_filing_analysis_orchestration_failure(
        ...
        mock_task_service.create_task.return_value = mock_task_response
        mock_analysis_orchestrator.validate_filing_access.return_value = True
        mock_analysis_orchestrator.orchestrate_filing_analysis.side_effect = Exception(
            "LLM failed"
        )
    ...
    async def test_execute_analysis_task_with_progress_callback(
        ...
        assert (
            len(mapped_progress_calls) >= 1
        )  # At least one progress call should be made
    ...
    async def test_retry_failed_task_success(
        ...
        with patch.object(
            coordinator, 'queue_filing_analysis', return_value=retry_task_response
        ) as mock_queue:
    ...
    async def test_full_workflow_integration(
        ...
        )  # At least initial validation and workflow start
    ...
    async def test_command_parameter_extraction(
        ...
        mock_task_service.create_task.return_value = mock_task_response
        mock_analysis_orchestrator.validate_filing_access.return_value = (
            False  # Skip full execution
        )
