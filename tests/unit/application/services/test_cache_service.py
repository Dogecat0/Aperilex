...
class TestCacheService:
    ...
    def test_generate_key_consistent_ordering(self, cache_service: CacheService) -> None:
    ...
    async def test_invalidate_analysis_cache(self, cache_service: CacheService) -> None:
        ...
        await cache_service.cache_analysis(analysis_id, {"data": 1}, include_full_results=True)
        await cache_service.cache_analysis(analysis_id, {"data": 2}, include_full_results=False)
        ...
        result1 = await cache_service.get_cached_analysis(analysis_id, include_full_results=True)
        result2 = await cache_service.get_cached_analysis(analysis_id, include_full_results=False)
    ...
    async def test_invalidate_filing_cache(self, cache_service: CacheService) -> None:
        ...
        result1 = await cache_service.get_cached_filing(filing_id, include_analyses=True)
        result2 = await cache_service.get_cached_filing(filing_id, include_analyses=False)
        assert result1 is None
        assert result2 is None
    ...
    def test_cache_statistics_error_handling(self, cache_service: CacheService) -> None:
        ...
        with patch.object(cache_service, '_estimate_cache_size_mb', side_effect=Exception("Mock error")):
            stats = cache_service.get_cache_statistics()
