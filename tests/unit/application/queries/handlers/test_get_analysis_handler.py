...
class TestGetAnalysisQueryHandler:
    ...
    async def test_handle_query_success_without_full_results(
        ...
        with patch.object(AnalysisResponse, 'from_domain', return_value=mock_analysis_response) as mock_from_domain:
        ...
        mock_analysis_repository.get_by_id.assert_called_once_with(sample_query.analysis_id)
        
        # Verify response conversion was called with correct parameters
        mock_from_domain.assert_called_once_with(mock_analysis, include_full_results=False)
    ...
    async def test_handle_query_success_with_full_results(
        ...
        mock_from_domain.assert_called_once_with(mock_analysis, include_full_results=True)
    ...
    async def test_handle_query_analysis_not_found(
        ...
        with pytest.raises(ValueError, match=f"Analysis with ID {sample_query.analysis_id} not found"):
    ...
    async def test_handle_query_response_conversion_error(
        ...
        with patch.object(AnalysisResponse, 'from_domain', side_effect=conversion_error):
    ...
    async def test_handle_query_different_detail_levels(
        ...
        for include_full, include_section, include_processing in detail_combinations:
            ...
            mock_from_domain.assert_called_once_with(mock_analysis, include_full_results=include_full)
    ...
    async def test_handle_query_different_user_ids(
        ...
        for user_id in user_ids:
            ...
            with patch.object(AnalysisResponse, 'from_domain', return_value=mock_analysis_response):
                result = await handler.handle(query)
    ...
    async def test_handle_query_logging_success(
        ...
        with patch.object(AnalysisResponse, 'from_domain', return_value=mock_analysis_response), \
             patch('src.application.queries.handlers.get_analysis_handler.logger') as mock_logger:
        ...
        assert initial_extra["include_full_results"] == sample_query.include_full_results
    ...
    async def test_handle_query_logging_error(
        ...
        with patch('src.application.queries.handlers.get_analysis_handler.logger') as mock_logger:
    ...
    async def test_integration_with_realistic_data(
        ...
        realistic_analysis = Analysis(
            ...
            metadata={
                ...
                "schemas_used": ["BusinessAnalysisSection", "RiskFactorsAnalysisSection"],
        ...
        with patch.object(AnalysisResponse, 'from_domain', return_value=realistic_response) as mock_from_domain:
        ...
        mock_from_domain.assert_called_once_with(realistic_analysis, include_full_results=True)
