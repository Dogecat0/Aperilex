...
class TestBaseQuery:
    ...
    def test_query_creation_with_explicit_values(self):
        ...
        query = SimpleQuery(
            user_id=user_id, page=page, page_size=page_size, filter_value="custom"
        )
    ...
    def test_query_equality_and_hashing(self):
        ...
        query1 = SimpleQuery(user_id="user1", page=1, page_size=20, filter_value="test")
        query2 = SimpleQuery(user_id="user1", page=1, page_size=20, filter_value="test")
        query3 = SimpleQuery(
            user_id="user1", page=2, page_size=20, filter_value="test"  # Different page
        )
    ...
    def test_complex_query_example(self):
        ...
        with pytest.raises(
            ValueError, match="At least one search criterion is required"
        ):
            FilingSearchQuery()
