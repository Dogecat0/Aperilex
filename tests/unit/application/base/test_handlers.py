...
class CreateUserCommandHandler(CommandHandler[CreateUserCommand, UserResult]):
    ...
    async def handle(self, command: CreateUserCommand) -> UserResult:
        ...
        user = UserResult(user_id=str(uuid4()), name=command.name, email=command.email)
...
class SearchUsersQueryHandler(QueryHandler[SearchUsersQuery, list[UserResult]]):
    ...
    async def handle(self, query: SearchUsersQuery) -> list[UserResult]:
        ...
        if query.name_filter:
            results = [
                u for u in results if query.name_filter.lower() in u.name.lower()
            ]

        if query.email_filter:
            results = [
                u for u in results if query.email_filter.lower() in u.email.lower()
            ]
...
class TestCommandHandler:
    ...
    async def test_create_user_handler_duplicate_email(self):
        ...
        # Create first user
        command1 = CreateUserCommand(name="John Doe", email="john@example.com")
        await handler.handle(command1)

        # Try to create second user with same email
        command2 = CreateUserCommand(
            name="Jane Doe", email="john@example.com"  # Same email
        )

        with pytest.raises(
            ValueError, match="User with email john@example.com already exists"
        ):
    ...
    async def test_update_user_handler(self):
        ...
        # Setup existing user
        user = UserResult(user_id="user123", name="John Doe", email="john@example.com")
        handler.users["user123"] = user

        # Update user name
        command = UpdateUserCommand(user_id="user123", name="John Smith")
    ...
    async def test_update_user_handler_not_found(self):
        ...
        command = UpdateUserCommand(user_id="nonexistent", name="New Name")
