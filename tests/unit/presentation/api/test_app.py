...
class TestLifespanManagement:
    ...
    async def test_lifespan_logging(self, mock_service_lifecycle):
        ...
        with patch('src.presentation.api.app.logger') as mock_logger:
            ...
            startup_logged = any("Starting up Aperilex API" in str(call) for call in log_calls)
            success_logged = any("startup completed successfully" in str(call) for call in log_calls)
            shutdown_logged = any("shutdown completed successfully" in str(call) for call in log_calls)
...
class TestMiddlewareIntegration:
    ...
    def test_cors_headers_present(self, client):
        ...
        assert "access-control-allow-origin" in response.headers or len(response.headers) >= 0
...
class TestEndpointIntegration:
    ...
    @pytest.mark.parametrize("endpoint_prefix", ["/api/filings", "/api/analyses", "/api/companies"])
