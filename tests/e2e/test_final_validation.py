...
async def test_subsection_analysis_features():
    ...
    try:
        ...
        if (
            analysis_field.get("type") == "object"
            and analysis_field.get("additionalProperties") is False
        ):
    ...
    try:
        ...
        if (
            serialized["analysis"]["executive_overview"]
            == sample_analysis["executive_overview"]
        ):
    ...
    try:
        ...
        print(
            f"    - First sub-section type: {section_response.sub_sections[0].schema_type}"
        )

        # Check that analysis content is accessible
        first_sub = section_response.sub_sections[0]
        if (
            isinstance(first_sub.analysis, dict)
            and "executive_overview" in first_sub.analysis
        ):
