...
class TestOpenAIProviderErrorHandling:
    ...
    def setup_both_methods_mock(self, provider, create_response=None, parse_response=None, 
                               create_side_effect=None, parse_side_effect=None):
        """Set up mocks for both create and parse methods with proper structure."""
        create_mock = patch.object(provider.client.chat.completions, 'create', new_callable=AsyncMock)
        parse_mock = patch.object(provider.client.chat.completions, 'parse', new_callable=AsyncMock)
    ...
    def provider(self):
        ...
        with patch('src.infrastructure.llm.openai_provider.AsyncOpenAI') as mock_client_class:
    ...
    async def test_rate_limit_error_handling(self, provider, sample_filing_data):
        ...
        with patch.object(provider.client.chat.completions, 'create', new_callable=AsyncMock) as mock_create, \
             patch.object(provider.client.chat.completions, 'parse', new_callable=AsyncMock) as mock_parse:
            ...
            subsection_text_response.choices[0].message.content = "Extracted subsection text content"
            ...
            def parse_side_effect(*args, **kwargs):
                ...
                if hasattr(response_format, '__name__') and response_format.__name__ == 'SectionSummaryResponse':
    ...
    async def test_api_connection_error_handling(self, provider, sample_filing_data):
        ...
        with patch.object(provider.client.chat.completions, 'create', new_callable=AsyncMock) as mock_create, \
            ...
            mock_parse.side_effect = APIConnectionError(message="Connection failed", request=mock_request)
    ...
    async def test_api_timeout_error_handling(self, provider, sample_filing_data):
        ...
        with patch.object(provider.client.chat.completions, 'create', new_callable=AsyncMock) as mock_create, \
            ...
            success_create = Mock(choices=[Mock(message=Mock(content=json.dumps({
                "section_name": "Item 1 - Business",
                "section_summary": "Test section summary",
                "consolidated_insights": ["Test insight 1", "Test insight 2"],
                "overall_sentiment": 0.5,
            ...
            success_parse = Mock(choices=[Mock(message=Mock(content=json.dumps({
                "filing_summary": "Brief filing summary",
                "executive_summary": "Test executive summary",
                "key_insights": ["Insight 1", "Insight 2"],
                "financial_highlights": ["Highlight 1", "Highlight 2"],
    ...
    async def test_invalid_json_response_handling(self, provider, sample_filing_data):
        ...
        with patch.object(provider.client.chat.completions, 'create', new_callable=AsyncMock) as mock_create, \
            ...
            def create_side_effect(*args, **kwargs):
                return Mock(choices=[Mock(message=Mock(content="Invalid JSON response"))])
            ...
            try:
                ...
                assert any(keyword in error_message for keyword in ["parse", "json", "validation", "retry"])
    ...
    async def test_empty_response_handling(self, provider, sample_filing_data):
        ...
        with patch.object(provider.client.chat.completions, 'create', new_callable=AsyncMock) as mock_create, \
            ...
            try:
                ...
                assert "response" in str(e).lower() or "choice" in str(e).lower() or "index" in str(e).lower()

    @pytest.mark.asyncio
    async def test_concurrent_request_error_handling(self, provider, sample_filing_data):
    ...
    async def test_retry_mechanism_exhaustion(self, provider, sample_filing_data):
        ...
        with patch.object(provider.client.chat.completions, 'create', new_callable=AsyncMock) as mock_create, \
            ...
            assert retry_call_count[0] >= 3  # At least 3 attempts due to retry mechanism
    ...
    async def test_section_analysis_error_recovery(self, provider):
        ...
        sample_filing_data = FilingData(
            ...
            cik="0000320193",
            content_text="Sample filing content for testing...",
            sections={"Item 1 - Business": "Apple Inc. designs, manufactures and markets smartphones..."}
        )
        ...
        with patch.object(provider.client.chat.completions, 'create', new_callable=AsyncMock) as mock_create, \
            ...
            success_create = Mock(choices=[Mock(message=Mock(content=json.dumps({
                ...
                "consolidated_insights": ["Recovered insight 1", "Recovered insight 2"],
                "overall_sentiment": 0.5,
                "critical_findings": ["Recovered finding 1", "Recovered finding 2"]
            })))])
            
            success_parse = Mock(choices=[Mock(message=Mock(content=json.dumps({
                "filing_summary": "Brief filing summary",
                "executive_summary": "Recovered executive summary",
                "key_insights": ["Recovered insight 1", "Recovered insight 2"],
                "financial_highlights": ["Recovered highlight 1", "Recovered highlight 2"],
                "risk_factors": ["Recovered risk 1", "Recovered risk 2"],
                "opportunities": ["Recovered opportunity 1", "Recovered opportunity 2"],
    ...
    async def test_large_content_truncation_handling(self, provider):
        ...
        with patch.object(provider.client.chat.completions, 'create', new_callable=AsyncMock) as mock_create, \
            ...
            # Use callable side effects to handle multiple calls
            def create_side_effect(*args, **kwargs):
                return Mock(choices=[Mock(message=Mock(content="Extracted large content text"))])
            ...
            def parse_side_effect(*args, **kwargs):
                ...
                if hasattr(response_format, '__name__') and response_format.__name__ == 'SectionSummaryResponse':
                    return Mock(choices=[Mock(message=Mock(content=json.dumps({
                        ...
                        "consolidated_insights": ["Large content insight 1", "Large content insight 2"],
                        "overall_sentiment": 0.5,
                        "critical_findings": ["Large content finding 1", "Large content finding 2"]
                    })))])
                else:
                    return Mock(choices=[Mock(message=Mock(content=json.dumps({
                        "filing_summary": "Brief filing summary for large content",
                        "executive_summary": "Test executive summary for large content",
                        "key_insights": ["Large insight 1", "Large insight 2"],
                        "financial_highlights": ["Large highlight 1", "Large highlight 2"],
                        "risk_factors": ["Large risk 1", "Large risk 2"],
                        "opportunities": ["Large opportunity 1", "Large opportunity 2"],
                        "confidence_score": 0.7
                    })))])
    ...
    async def test_invalid_template_handling(self, provider, sample_filing_data):
        ...
        with patch.object(provider.client.chat.completions, 'create', new_callable=AsyncMock) as mock_create, \
            ...
            # Use callable side effects to handle multiple calls
            def create_side_effect(*args, **kwargs):
                return Mock(choices=[Mock(message=Mock(content="Extracted template text"))])
            ...
            def parse_side_effect(*args, **kwargs):
                ...
                if hasattr(response_format, '__name__') and response_format.__name__ == 'SectionSummaryResponse':
                    return Mock(choices=[Mock(message=Mock(content=json.dumps({
                        "section_name": "Item 1 - Business",
                        "section_summary": "Test section summary",
                        "consolidated_insights": ["Template insight 1", "Template insight 2"],
                        "overall_sentiment": 0.0,
                        "critical_findings": ["Template finding 1", "Template finding 2"]
                    })))])
                else:
                    return Mock(choices=[Mock(message=Mock(content=json.dumps({
                        "filing_summary": "Brief filing summary for invalid template",
                        "executive_summary": "Test executive summary for invalid template",
                        "key_insights": ["Template insight 1", "Template insight 2"],
                        "financial_highlights": ["Template highlight 1", "Template highlight 2"],
                        "risk_factors": ["Template risk 1", "Template risk 2"],
                        "opportunities": ["Template opportunity 1", "Template opportunity 2"],
                        "confidence_score": 0.7
                    })))])
    ...
    @pytest.mark.asyncio
    async def test_concurrent_analysis_resource_management(self, provider, sample_filing_data):
        ...
        large_filing = FilingData(
            ...
            sections={f"section_{i}": f"Content {i}" for i in range(10)}  # Reduced from 20 to 10
        )
        ...
        with patch.object(provider.client.chat.completions, 'create', new_callable=AsyncMock) as mock_create, \
            ...
            def create_side_effect(*args, **kwargs):
                return Mock(choices=[Mock(message=Mock(content="Extracted concurrent text"))])
            ...
            def parse_side_effect(*args, **kwargs):
                ...
                if hasattr(response_format, '__name__') and response_format.__name__ == 'SectionSummaryResponse':
                    return Mock(choices=[Mock(message=Mock(content=json.dumps({
                        ...
                        "consolidated_insights": ["Concurrent insight 1", "Concurrent insight 2"],
                        "overall_sentiment": 0.5,
                        "critical_findings": ["Concurrent finding 1", "Concurrent finding 2"]
                    })))])
                else:
                    return Mock(choices=[Mock(message=Mock(content=json.dumps({
                        "filing_summary": "Brief filing summary for concurrent test",
                        "executive_summary": "Test executive summary for concurrent analysis",
                        "key_insights": ["Concurrent insight 1", "Concurrent insight 2"],
                        "financial_highlights": ["Concurrent highlight 1", "Concurrent highlight 2"],
                        "risk_factors": ["Concurrent risk 1", "Concurrent risk 2"],
                        "opportunities": ["Concurrent opportunity 1", "Concurrent opportunity 2"],
                        "confidence_score": 0.7
                    })))])
