...
import json
from src.infrastructure.llm.base import SectionAnalysisResponse, SectionSummaryResponse, SubsectionAnalysisResponse
...
def test_openai_schema_compatibility():
    ...
    try:
        ...
        if schema.get("additionalProperties") is False:
            ...
            print(f"  ✗ ISSUE: Root schema additionalProperties is {schema.get('additionalProperties')}, should be false")
        
        # Check that all required fields are present
        required_fields = schema.get('required', [])
        expected_fields = ['section_name', 'section_summary', 'consolidated_insights', 'overall_sentiment', 'critical_findings']
        missing_fields = [field for field in expected_fields if field not in required_fields]
        ...
        if 'properties' in schema and 'analysis' in schema['properties']:
            ...
            if analysis_field.get('type') == 'object':
                ...
                if 'additionalProperties' in analysis_field:
                    print(f"  ✓ Analysis field additionalProperties: {analysis_field['additionalProperties']}")
                else:
                    print("  ⚠️  Analysis field doesn't specify additionalProperties")
            else:
                print(f"  ✗ ISSUE: Analysis field type is {analysis_field.get('type')}, should be object")
        else:
    ...
    try:
        ...
        print(f"  ✓ Schema JSON serialization successful ({len(schema_json)} characters)")
