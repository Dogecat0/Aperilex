...
class TestTaskStatusEndpoint:
    ...
    def test_get_task_status_success(
        ...
        mock_background_task_coordinator.get_task_status.return_value = (
            sample_task_response
        )
        ...
        assert data["result"] == {
            "analysis_id": sample_task_response.result["analysis_id"]
        }
        assert data["error_message"] is None

        # Verify coordinator was called with correct task_id
        mock_background_task_coordinator.get_task_status.assert_called_once_with(
            task_id
        )

    def test_get_task_status_completed(
        self, test_client_with_task_coordinator, mock_background_task_coordinator
    ):
    ...
    def test_get_task_status_coordinator_exception(
        ...
        mock_background_task_coordinator.get_task_status.side_effect = Exception(
    ...
    def test_get_task_status_invalid_uuid(
        ...
        response = test_client_with_task_coordinator.get(
            f"/api/tasks/{invalid_task_id}/status"
        )
...
class TestTaskRetryEndpoint:
    ...
    def test_retry_failed_task_success(
        ...
        mock_background_task_coordinator.retry_failed_task.return_value = (
            retry_task_response
        )

        original_task_id = sample_task_response.task_id
        response = test_client_with_task_coordinator.post(
            f"/api/tasks/{original_task_id}/retry"
        )
        ...
        # Verify coordinator was called with correct task_id
        mock_background_task_coordinator.retry_failed_task.assert_called_once_with(
            original_task_id
        )
    ...
    def test_retry_task_coordinator_exception(
        ...
        mock_background_task_coordinator.retry_failed_task.side_effect = Exception(
...
class TestTaskEndpointsIntegration:
    ...
    def test_task_lifecycle_workflow(
        ...
        mock_background_task_coordinator.get_task_status.return_value = (
            completed_retry_response
        )

        response = test_client_with_task_coordinator.get(
            f"/api/tasks/{retry_task_id}/status"
        )
        assert response.status_code == 200
    ...
    def test_endpoint_path_parameters(
        ...
        for task_id in test_cases:
            ...
            status_response = test_client_with_task_coordinator.get(
                f"/api/tasks/{task_id}/status"
            )
            retry_response = test_client_with_task_coordinator.post(
